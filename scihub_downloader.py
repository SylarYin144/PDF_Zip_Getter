...
def download_from_google_scholar(query, session):
    ...
    try:
        ...
        for link_tag in soup.find_all('a', href=True):
            ...
            if href.lower().endswith('.pdf'): potential_links.append(href)
        for link_tag in soup.find_all('div', class_='gs_ggs'):
            s_link = link_tag.find('a', href=True)
            if s_link and s_link['href'].lower().endswith('.pdf'): potential_links.append(s_link['href'])
        ...
        return None, "FALLO - No se encontró PDF en Google Scholar"
...
def download_from_pmc(query, session):
    ...
    try:
        ...
        if not pmcid:
            ...
            article_link = soup_search.find('div', class_='rprt').find('a');
            if not article_link: return None, "FALLO - No se encontró artículo en PMC"
            article_url = urljoin(search_url, article_link['href'])
        ...
        for link_tag in soup.select('div.format-menu a[href$=".pdf"], .pdf-btn a[href$=".pdf"]'):
            ...
            if 'application/pdf' in pdf_response.headers.get('Content-Type', ''): return pdf_response.content, "PMC"
...
class SciHubDownloaderApp:
    def __init__(self, root):
        self.root = root; self.root.title("Sci-Hub Downloader Pro"); self.root.geometry("1200x800"); self.root.minsize(800, 700)
        ...
        self.time_elapsed_str = tk.StringVar(value="Transcurrido: 0s"); self.time_avg_doi_str = tk.StringVar(value="Promedio/DOI: 0s"); self.time_remaining_str = tk.StringVar(value="Tiempo Restante: -")
    ...
    def _create_progress_widgets(self):
        ...
        self.stats_label = ttk.Label(metrics_frame, textvariable=self.stats_text_str, justify=tk.CENTER); self.stats_label.grid(row=2, column=0, columnspan=3, sticky="ew", pady=5)
    ...
    def _process_queue(self):
        try:
            while True:
                ...
                elif msg_type == 'progress': self.progress_bar['value'] = data['percentage']; self.progress_article_str.set(f"Artículo: {data['current']}/{data['total']} ({data['percentage']:.1f}%)"); self.time_elapsed_str.set(f"Transcurrido: {timedelta(seconds=int(data['elapsed']))}"); self.time_avg_doi_str.set(f"Promedio/DOI: {data['avg_time']:.1f}s"); self.time_remaining_str.set(f"Tiempo Restante: {timedelta(seconds=int(data['remaining']))}")
                ...
                elif msg_type == 'article_status': self.article_treeview.item(data['row_id'], values=(data['row_id'] + 1, data['doi'], data['title'], data['status']), tags=(data['tag'],)); self._update_pie_charts(data['stats']); self.stats_text_str.set(f"Buscados: {data['stats']['successful'] + data['stats']['failed']}/{data['stats']['successful'] + data['stats']['failed'] + data['stats']['pending']} | Obtenidos: {data['stats']['successful']} | Fallidos: {data['stats']['failed']} | Pendientes: {data['stats']['pending']}")
        ...
        try:
            ...
            with zipfile.ZipFile(config['zip_output'], 'w', zipfile.ZIP_DEFLATED) as zf:
                for index, row in self.df_articles.iterrows():
                    ...
                    current_num = index + 1; elapsed = time.time() - start_time_total; avg_time = elapsed / current_num if current_num > 0 else 0
                    ...
                    if query_term:
                        ...
                        if config['use_scihub']:
                            for mirror in config['mirrors']:
                                ...
                                if pdf_content: source = "Sci-Hub"; break
