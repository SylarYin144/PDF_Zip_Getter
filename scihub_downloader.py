...
# --- Helper Functions from User's working script ---
            ...
            with zipfile.ZipFile(config['zip_output'], 'w', zipfile.ZIP_DEFLATED) as zf:
                for index, row in self.df_articles.iterrows():
                    ...
                    if query_term:
                        ...
                        if not pdf_content and config['use_pmc']: pdf_content, reason = download_from_pmc(query_term, session); source = "PMC" if pdf_content else source
                        if not pdf_content and config['use_google_scholar']: pdf_content, reason = download_from_google_scholar(query_term, session); source = "Google Scholar" if pdf_content else source
